...
class DLCLive(object):
    ...
    def init_inference(self, frame=None, **kwargs):
        ...
        if self.model_type == 'base':
            ...
            try:
                ...
                raise DLCLiveError(('This model cannot be converted to tensorflow lite format. '
            ...
            raise DLCLiveError("model_type = {} is not supported. model_type must be 'base', 'tflite', or 'tensorrt'".format(self.model_type))
    ...
    def get_pose(self, frame=None, **kwargs):
        ...
        if self.model_type in ['base', 'tensorrt']:
            ...
            pose_output = self.sess.run(self.outputs, feed_dict={
                                        self.inputs: np.expand_dims(frame, axis=0)
            })

        elif self.model_type == 'tflite':

            self.tflite_interpreter.set_tensor(self.inputs[0]['index'],
        ...
        if len(pose_output) > 1:
            ...
            if num_outputs > 1:
                self.pose = multi_pose_predict(scmap, locref, self.cfg['stride'], num_outputs)
            else:
